...
int sendresponse(char* mac,char* dest_mac,char* ip,char* dest)
{
    Ether_pkg pkg;
    struct hostent *host =NULL;
    struct sockaddr sa;
    ...
    /* 填充ethernet包文 */
    memcpy ((char*)pkg.ether_shost, (char*)mac, 6);
    memcpy ((char*)pkg.ether_dhost, (char*)dest_mac, 6);
    pkg.ether_type = htons(ETHERTYPE_ARP);
    ...
    memcpy((char*)pkg.arp_sha, (char*)mac, 6);
    memcpy((char*)pkg.arp_spa, (char*)ip, 4);
    memcpy((char*)pkg.arp_tha, (char*)dest_mac, 6);

    printf ( "Resolve [%s],Please Waiting...",dest );

    /*以欺骗方式发包，会造成IP冲突错误 */
    fflush(stdout);
    memset( p,'\0',sizeof(ip) ;
    if (inet_aton(dest, (struct in_addr*)ip) == 0)
    {
        if ((host = gethostbyname(dest)) ==NULL )
        {
...
int main ( int argc,char **argv )
{
    ...
    char dest[16]={0};
    // 0:c:29:2e:b9:6e
    unsigned char dest_mac[7]={0x00,0x0c,0x29,0x2e,0xb9,0x6e,0x00};
    //以太网帧首部的硬件地址填FF:FF:FF:FF:FF:FF表示广播
    memset(mac, 0, sizeof(mac));
    memset(ip, 0, sizeof(ip));
    if ( GetLocalMac("ens33", mac, ip) ==-1)
        return ( -1 );
